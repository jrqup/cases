...
function getCart(){
	...
	@$uid=$_SESSION["uid"];
	if($uid){
		$sql="select iid, lid, title, spec, price, count ,(select sm from xz_laptop_pic where laptop_id=lid limit 1) as sm ,is_checked from xz_shoppingcart_item inner join xz_laptop on product_id=lid where user_id=$uid";
		$result=mysqli_query($conn,$sql);
